...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Add or update Jalkiruoka (menu item)
    ...
    if ($id) {
        ...
        $sql = "UPDATE jalkiruoka SET name = '$name', tietoa = '$tietoa', hinta = '$hinta' WHERE id = '$id'";
        if ($conn->query($sql) === TRUE) {
            echo json_encode(["status" => "success", "message" => "Jalkiruoka päivitetty"]);
        } else {
            echo json_encode(["status" => "error", "message" => "Virhe päivittäessä tietoja: " . $conn->error]);
        }
    } else {
        // Insert new item
        $sql = "INSERT INTO jalkiruoka (name, tietoa, hinta) VALUES ('$name', '$tietoa', '$hinta')";
        if ($conn->query($sql) === TRUE) {
            echo json_encode(["status" => "success", "message" => "Jalkiruoka lisätty onnistuneesti"]);
        } else {
            echo json_encode(["status" => "error", "message" => "Virhe lisättäessä tietoja: " . $conn->error]);
        }
    }
} elseif ($_SERVER['REQUEST_METHOD'] === 'GET') {
    // Retrieve all Jalkiruoka items
    $sql = "SELECT * FROM jalkiruoka";
    ...
    echo json_encode($jalkiruoka);
} elseif ($_SERVER['REQUEST_METHOD'] === 'DELETE') {
    // Delete a specific Jalkiruoka item
    $id = isset($_GET['id']) ? $conn->real_escape_string($_GET['id']) : null;

    if ($id) {
        $sql = "DELETE FROM jalkiruoka WHERE id = '$id'";
        if ($conn->query($sql) === TRUE) {
            echo json_encode(["status" => "success", "message" => "Jalkiruoka poistettu"]);
        } else {
